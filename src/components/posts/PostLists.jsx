...
import DataTablePost from '../common/DataTablePost'
...
const PostList = () => {
    ...
    const onSelectAll = (e) => {
        if(e.target.checked) {
            const temp = posts.map(user => user.id)
            setSelected(temp)
        } else {
            setSelected([])
        }
    ...
    const handleDeleteApi = () => {
        if(deleteType == 'single') {
            ...
            dispatch(actions.controlLoading(true))
            requestApi(`posts/multiple?ids=${selected.toString()}`,'DELETE',[])
            .then(res => {
                setShowPopup(false)
                dispatch(actions.controlLoading(false))
                setSelected([])
        ...
        <div id="layoutSidenav_content">
            <main>
                <div className='container-fluid px-4'>
                    ...
                    <div className='mb-3'>
                        <Link to="/posts/add" className='btn btn-sm btn-success me-2'><i className='fa fa-plus'></i>Add new</Link>
                        {selected.length > 0 ? (
